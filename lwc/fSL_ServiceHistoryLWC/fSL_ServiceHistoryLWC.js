...
import serviceHistoryClosed from '@salesforce/label/c.FSL_Service_History_Closed';
import serviceHistoryCancelled from '@salesforce/label/c.FSL_Service_History_Canceled';
import closedLabel from '@salesforce/label/c.CSS_Closed';
import canceledLabel from '@salesforce/label/c.CSS_Canceled';
import serviceHistoryLabel from '@salesforce/label/c.CSS_Service_History';
...
import getWorkOrders from '@salesforce/apex/FSL_ServiceHIstoryController.getWorkOrders';
...
export default class DataTableWithSortingInLWC extends LightningElement {
    ...
    contacts(result) {
        if (result.data) {
            ...
            this.workNotStartedButtonLabel = closedLabel + ' | ' + result.data.workNotStarted.length;
            this.WorkInProgressButtonLabel = canceledLabel + ' | ' + result.data.workInProgress.length;
            const totalResultLength = result.data.workNotStarted.length + result.data.workInProgress.length;
            this.ResultSize = serviceHistoryLabel + ' | ' + totalResultLength;
        ...
        if (this.toatalData) {
            ...
            this.toatalData.forEach((row) => {
                ...
                if (row.Repair_Site_Name__c) {
                    rowData.RepairSiteName = row.FSL_Service_Location__c;
                }
                if (row.Asset) {
    ...
    WorkNotStarted(event) {
        ...
        this.WorkTableTitle = serviceHistoryClosed;
